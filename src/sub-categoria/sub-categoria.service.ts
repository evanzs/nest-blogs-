...
export class SubCategoriaService {
  ...
  async create(createSubCategoriaDto: CreateSubCategoriaDto): Promise<any> {
    const entity = await this.repository.create(createSubCategoriaDto);

    if (createSubCategoriaDto.id_pai) {
      const pai = await this.categoriaRepository.findOne(
        createSubCategoriaDto.id_pai,
      );

      if (pai) entity.categoria = pai;
    }
