...
export class RespostaService {
  constructor(
    @InjectRepository(RespostaRepository)
  ...
  async create(createRespostaDto: CreateRespostaDto): Promise<any> {
    const entity = await this.repository.create(createRespostaDto);

    if (createRespostaDto?.id_topico) {
      const topico = await this.topicoRepository.findOne(
        createRespostaDto.id_topico,
      );

      if (topico) entity.topico = topico;
    }

    if (createRespostaDto?.id_usuario) {
      const user = await this.userRepository.findOne(
        createRespostaDto.id_usuario,
      );

      if (user) entity.usuario = user;
    }
